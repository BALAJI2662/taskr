        ...
        actions={(
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              disabled={day === today}
              className="rounded-lg border border-border px-2.5 py-1.5 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:opacity-50"
            >
              Today
            </button>
            <button
      ...
      <div className="card">
        <header className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-4 sm:px-5">
          <h2 className="flex items-center gap-2 font-semibold text-foreground">
            ...
            {formatDate(day)}
            {day === today && <span className="text-sm font-normal text-primary-strong">· today</span>}
          </h2>
          {todos.length > 0 && (
        ...
        <form onSubmit={add} className="filter-bar space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="hint">
                {myTasks.length === 0
                  ? <>You have no tasks yet — <Link to={taskListHref} className="text-primary-strong hover:underline">your task list</Link> is empty.</>
          ...
          <ul className="stagger divide-y divide-border">
            {todos.map((todo) => (
              <li key={todo.id} className="group flex items-start gap-3 px-4 py-3.5 sm:px-5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {(todo.project_key || todo.task_key) && (
                    <p className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground">
                      {todo.project_key && (
                        <span className="rounded bg-primary/10 px-1.5 py-0.5 font-mono font-bold uppercase tracking-wide text-primary-strong">
                          {todo.project_key}
                        </span>
                      )}
                      {todo.task_key && (
                        <Link
                          ...
                          className="truncate hover:text-primary-strong hover:underline"
                        >
                ...
                {busyId === todo.id ? (
                  <Spinner className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
                ) : (
                  <button
                    type="button"
